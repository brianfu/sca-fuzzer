...
bool enable_ddp_prefetcher = ENABLE_DDP_DEFAULT;
...
/// Control enable DDP prefetcher
///
static ssize_t enable_ddp_prefetcher_store(struct kobject *kobj, struct kobj_attribute *attr,
                                       const char *buf, size_t count);
static struct kobj_attribute enable_ddp_prefetcher_attribute =
    __ATTR(enable_ddp_prefetcher, 0666, NULL, enable_ddp_prefetcher_store);
...
static struct attribute *sysfs_attributes[] = {
    ...
    &enable_ddp_prefetcher_attribute.attr,
                                       ...
                                       const char *buf, size_t count)
{
    unsigned value = 0;
    sscanf(buf, "%u", &value);
    enable_ddp_prefetcher = (value == 0) ? false : true;
    return count;
}
...
static ssize_t dbg_dump_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    len += sprintf(&buf[len], "enable_ddp_prefetcher: %d\n", enable_ddp_prefetcher);
